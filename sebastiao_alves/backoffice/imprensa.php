...
$item = selectSQL("SELECT * FROM imprensa ORDER BY SUBSTRING_INDEX(SUBSTRING_INDEX(data_pub, ' ', -1), ' ', 1) DESC;");


if(isset($_GET["apagar"])){
    $imprensa_especifica = selectSQLUnico("SELECT * FROM imprensa WHERE id='$_GET[apagar]'");
    if(!empty($imprensa_especifica)){
        iduSQL("DELETE FROM imprensa WHERE id='$_GET[apagar]'");
    }
                ...
                <?php if(!isset($_GET["apagar"])): ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td> <img src="<?= $i["img_imprensa"] ?>" alt=""></td>
                                <td><?= $i["titulo"] ?></td>
                                <td><?= $i["texto"] ?></td>
                                <td><?= $i["data_pub"] ?></td>
                                <td> 
                    ...
                    <a href="criar_imprensa.php">
                        <button id="botesp">Criar Nova Postagem</button>
                    </a>

                    <?php else: ?>
                        <h3>Postagem apagada!</h3>
